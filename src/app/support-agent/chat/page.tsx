...
export default function AgentLiveChat() {
    ...
    const handleManualKyc = async () => {
        ...
        startKycTransition(async () => {
            ...
            if (res.success) {
                toast({ title: "KYC Verified & Activated", description: "User account is now live." });
                        ...
                        conversations.map(c => (
                            ...
                            >
                                <div className="min-w-0">
                                    ...
                                    <p className="text-[11px] text-gray-500 font-medium truncate italic mt-0.5">{c.last_message_preview || 'No messages yet...'}</p>
                                </div>
                                <div className="flex flex-col items-end gap-1.5 shrink-0 ml-2">
                                    {c.unread_count_admin > 0 && <Badge className="bg-primary text-white h-5 min-w-5 flex items-center justify-center rounded-full text-[10px] animate-pulse">{c.unread_count_admin}</Badge>}
            ...
            <div className="flex-grow flex flex-col relative bg-slate-950 overflow-hidden">
                {!activeConversation ? (
                    <div className="flex-grow flex flex-col items-center justify-center text-center p-12">
                        ...
                        <p className="text-gray-500 text-sm font-medium max-w-xs mt-2">Select a session to start assistance.</p>
                    </div>
                ) : (
                    <>
                        ...
                        <div ref={scrollRef} className="flex-grow p-6 overflow-y-auto bg-slate-950 scroll-smooth">
                                ...
                                {messages.map(m => (
                                    <div key={m.id} className={cn("flex items-end gap-3", m.sender_role === 'admin' ? "flex-row-reverse" : "flex-row")}>
                                        <div className={cn("max-w-[75%] p-4 rounded-2xl text-xs leading-relaxed shadow-lg", m.sender_role === 'admin' ? "bg-primary text-white rounded-br-none" : "bg-white/5 border border-white/5 text-gray-300 rounded-bl-none")}>
                                            {m.image_url && <div className="mb-2 rounded-lg overflow-hidden border border-white/10"><Image src={m.image_url} alt="Support Image" width={250} height={250} className="object-cover" /></div>}
                                            <p className="whitespace-pre-wrap">{m.message}</p>
                                            <p className="mt-2 text-[8px] opacity-30 font-bold text-right uppercase">{format(new Date(m.created_at), 'p')}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                        ...
                        <div className="p-5 bg-slate-900/80 border-t border-white/5 backdrop-blur-xl shrink-0">
                            <form onSubmit={handleSendMessage} className="flex gap-4 max-w-4xl mx-auto">
                                <Input placeholder="Type response..." value={messageText} onChange={(e) => setMessageText(e.target.value)} className="flex-grow bg-black/40 border-white/10 h-12 text-sm text-white rounded-xl focus:ring-primary/50" />
                    ...
                    <div className="p-6 space-y-8">
                        ...
                        <section className="space-y-4">
                            ...
                            <div className="space-y-3">
                                {userAccounts.map(acc => (
                                    <div key={acc.id} className="p-4 bg-black/20 rounded-2xl border border-white/5 group hover:bg-black/40 transition-colors">
                                        <p className="text-[11px] font-bold text-white truncate">{acc.plan_name}</p>
                                        <p className="text-[9px] text-gray-600 font-bold uppercase mt-1">Purchased: {format(new Date(acc.created_at), 'dd MMM yyyy')}</p>
                                        <div className="flex items-center justify-between mt-3">
                                            <Badge variant="outline" className="text-[8px] border-white/10 uppercase px-1.5">{acc.status}</Badge>
                        ...
                        {userInfo?.kyc_status !== 'verified' && (
                            <section className="space-y-4 pt-8 border-t border-white/5">
                                <h4 className="text-[10px] font-bold text-purple-400 uppercase tracking-widest flex items-center gap-2"><ShieldCheck className="w-3.5 h-3.5"/> Manual KYC Override</h4>
                                <div className="space-y-4 bg-purple-500/5 p-4 rounded-2xl border border-purple-500/10">
                                    <div className="space-y-2">
                                        <Label className="text-[9px] font-bold text-gray-500 uppercase">Aadhaar / ID Card</Label>
                                        <div className="relative">
                                            <input type="file" id="kyc1" className="hidden" onChange={(e) => setKycImg1(e.target.files?.[0] || null)} />
                                            <Button variant="outline" size="sm" className="w-full h-8 text-[10px] border-dashed border-white/10 bg-black/20" onClick={() => document.getElementById('kyc1')?.click()}>
                                                {kycImg1 ? <span className="text-green-400 truncate">{kycImg1.name}</span> : <><Upload className="w-3 h-3 mr-2"/> Upload ID</>}
                                            </Button>
                                        </div>
                                    </div>
                                    <div className="space-y-2">
                                        <Label className="text-[9px] font-bold text-gray-500 uppercase">Selfie with ID</Label>
                                        <div className="relative">
                                            <input type="file" id="kyc2" className="hidden" onChange={(e) => setKycImg2(e.target.files?.[0] || null)} />
                                            <Button variant="outline" size="sm" className="w-full h-8 text-[10px] border-dashed border-white/10 bg-black/20" onClick={() => document.getElementById('kyc2')?.click()}>
                                                {kycImg2 ? <span className="text-green-400 truncate">{kycImg2.name}</span> : <><Upload className="w-3 h-3 mr-2"/> Upload Selfie</>}
                                            </Button>
                                        </div>
                                    </div>
                                    <Button onClick={handleManualKyc} disabled={!kycImg1 || !kycImg2 || isKycPending} className="w-full bg-purple-600 hover:bg-purple-500 h-10 rounded-xl text-[10px] font-bold uppercase shadow-xl shadow-purple-900/20">
                                        {isKycPending ? <Loader2 className="animate-spin h-3.5 w-3.5"/> : <CheckCircle className="w-3.5 h-3.5 mr-2"/>}
                                        Verify & Activate
                                    </Button>
                                </div>
                            </section>
