...
import { Loader2, ArrowLeft, Send, Check, RefreshCw, Paperclip, User, Headphones } from 'lucide-react';
...
export default function AgentTicketDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    fetchData();
    const sub = supabase.channel(`agent_ticket_${id}`)
  ...
  const handleReplySubmit = async (e: React.FormEvent) => {
    ...
    setIsSending(true);
    const res = await sendSupportMessage(id, agentId!, 'admin', replyText);
  ...
  return (
    <div className="bg-slate-950 min-h-screen font-poppins flex flex-col h-screen">
        <header className="p-6 border-b border-white/5 bg-slate-900/50 backdrop-blur-md flex items-center justify-between sticky top-0 z-50 shrink-0">
           ...
           <div className="flex items-center gap-3">
                ...
                {ticket?.status === 'open' && (
                     <Button size="sm" onClick={handleCloseTicket} className="bg-white/5 hover:bg-white/10 border-white/10 text-white font-bold text-[10px] uppercase rounded-xl px-5 h-10">
                        <Check className="mr-2 h-3.5 w-3.5" /> Resolve Ticket
                    </Button>
                )}
           </div>
        </header>
        ...
        <ScrollArea ref={scrollRef} className="flex-1 p-6 md:p-10">
            <div className="max-w-4xl mx-auto space-y-10">
                <div className="space-y-6">
                    {messages.map((m, i) => (
                        <div key={i} className={cn("flex items-start gap-4", m.sender_role === 'admin' ? "flex-row-reverse" : "flex-row")}>
                            <Avatar className={cn("h-10 w-10 border-2 shadow-xl", m.sender_role === 'admin' ? "border-primary/20" : "border-white/10")}>
                                <AvatarFallback className={m.sender_role === 'admin' ? "bg-primary/20 text-primary font-bold" : "bg-slate-800 text-gray-400"}>
                                    {m.sender_role === 'admin' ? 'A' : 'T'}
                                </AvatarFallback>
                            </Avatar>
                            <div className={cn("max-w-xl rounded-2xl p-5 shadow-2xl", m.sender_role === 'admin' ? "bg-primary text-white" : "bg-white/5 border border-white/5 text-gray-200")}>
            ...
            <div className="p-6 bg-slate-900/80 border-t border-white/5 backdrop-blur-xl shrink-0">
                <form onSubmit={handleReplySubmit} className="max-w-4xl mx-auto space-y-4">
                    <Textarea
                        ...
                        placeholder="Provide an update to the trader..."
                        rows={3}
                        className="bg-black/40 border-white/10 text-white text-sm focus:ring-primary/50"
                    />
                    <div className="flex items-center justify-between">
                         <div className="flex items-center gap-4">
                            <Label htmlFor="agent-attach" className="flex items-center gap-2 text-[10px] font-bold text-gray-500 uppercase tracking-widest cursor-pointer hover:text-white transition-colors">
                                <Paperclip className="h-3.5 w-3.5" /> Attach Proof
                            </Label>
                            <Input id="agent-attach" type="file" accept="image/*" onChange={handleFileChange} className="sr-only" />
                            {preview && (
                        ...
                        <Button type="submit" disabled={isSending || (!replyText.trim() && !attachment)} className="bg-primary hover:bg-primary/90 text-white font-bold h-12 px-10 rounded-xl shadow-xl shadow-primary/20">
                            {isSending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Send className="mr-2 h-4 w-4" />} Send Response
                        </Button>
                    </div>
                </form>
            </div>
