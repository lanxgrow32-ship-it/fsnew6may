...
/**
 * Specialist Desk Protocol V2
 * A dedicated, separate command center for KYC & Payout specialist agents.
 */
export default function SpecialistPanel() {
    ...
    useEffect(() => {
        ...
        const sub = supabase.channel('spec_protocol_v2').on('postgres_changes', { event: '*', schema: 'public', table: 'support_conversations', filter: "assigned_role=eq.specialist" }, fetchSpecialistTickets).subscribe();
    ...
    return (
        <div className="min-h-screen bg-slate-950 p-4 md:p-8 space-y-8 animate-in fade-in duration-500 font-poppins text-white">
            {/* Command Header */}
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6 border-b border-white/5 pb-8">
                <div className="space-y-2">
                    <div className="flex items-center gap-3">
                        <div className="h-10 w-10 rounded-xl bg-green-500/20 text-green-400 flex items-center justify-center border border-green-500/20 shadow-[0_0_30px_rgba(34,197,94,0.1)]">
                            <ShieldCheck className="h-6 w-6"/>
                        </div>
                        <h2 className="text-3xl font-black tracking-tight text-white">Specialist Command</h2>
                    </div>
                    <p className="text-gray-500 text-xs font-bold uppercase tracking-[0.3em] flex items-center gap-2">
                        ...
                        Dedicated KYC & Payout Mitigation Grid
                    </p>
                </div>
                ...
                <div className="flex items-center gap-4 w-full lg:w-auto">
                    <div className="relative flex-grow lg:w-80">
                        ...
                        <Input 
                            placeholder="Filter protocol stream..." 
                            value={searchTerm}
                            onChange={(e) => setSearchTerm(e.target.value)}
                            className="pl-10 bg-black/40 border-white/10 h-12 text-xs font-bold uppercase tracking-widest rounded-2xl"
                        />
                    </div>
                    <div className="hidden md:flex flex-col items-end">
                        <p className="text-[10px] font-black text-gray-600 uppercase tracking-widest">Active Alerts</p>
                        <p className="text-lg font-black text-green-400">{filtered.length}</p>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                {loading ? (
                    <div className="col-span-full py-32 text-center">
                        <Loader2 className="animate-spin h-10 w-10 mx-auto text-green-500 opacity-50"/>
                    </div>
                ) : filtered.length > 0 ? (
                    filtered.map(t => (
                            ...
                            <Card className="bg-slate-900/50 border-white/5 hover:border-green-500/30 transition-all duration-300 rounded-3xl overflow-hidden relative shadow-2xl">
                                <CardHeader className="pb-4">
                                    <div className="flex justify-between items-start">
                                        <Badge variant="outline" className={cn(
                                            ...
                                            {t.escalation_reason || 'Manual'} Protocol
                                        </Badge>
                                        <p className="text-[9px] text-gray-600 font-bold uppercase">{formatDistanceToNow(new Date(t.last_message_at), { addSuffix: true })}</p>
                                    </div>
                                    <CardTitle className="text-xl font-black text-white mt-3 group-hover:text-green-400 transition-colors">{t.profiles?.full_name}</CardTitle>
                                ...
                                <CardContent className="space-y-4">
                                    <div className="p-4 bg-black/40 rounded-2xl border border-white/5">
                                        <p className="text-[10px] text-gray-600 font-black uppercase tracking-widest mb-2">Neural Handover Context</p>
                                ...
                                <CardFooter className="bg-white/[0.02] border-t border-white/5 py-4 flex justify-between items-center group-hover:bg-green-500/5 transition-all">
                                    ...
                                    <ArrowRight className="h-4 w-4 text-gray-700 group-hover:text-green-400 group-hover:translate-x-1 transition-all" />
                                </CardFooter>
                    ...
                    <div className="col-span-full py-40 text-center space-y-6 bg-white/[0.01] rounded-[40px] border border-dashed border-white/5">
                        ...
                        <div className="space-y-2">
                            <h3 className="text-2xl font-black text-white tracking-tighter">Protocol Standby</h3>
                            <p className="text-gray-500 text-xs font-bold uppercase tracking-[0.2em]">No technical escalations detected in the command grid.</p>
                        </div>
                    </div>
                )}
            </div>
