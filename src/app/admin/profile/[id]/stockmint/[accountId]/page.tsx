        ...
        <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <Card className="bg-black/40 border-white/5 shadow-2xl">
                    <CardHeader className="py-4"><CardTitle className="text-[10px] font-black uppercase text-gray-500 tracking-[0.3em]">Live Cash Balance</CardTitle></CardHeader>
                    <CardContent><p className="text-3xl font-black text-white">₹{syncData.balance?.toLocaleString()}</p></CardContent>
                </Card>
                <Card className="bg-black/40 border-white/5 shadow-2xl">
                    <CardHeader className="py-4"><CardTitle className="text-[10px] font-black uppercase text-gray-500 tracking-[0.3em]">High Water Mark</CardTitle></CardHeader>
                    <CardContent><p className="text-3xl font-black text-primary">₹{syncData.highWaterMark?.toLocaleString()}</p></CardContent>
                </Card>
                <Card className="bg-black/40 border-white/5 shadow-2xl">
                    <CardHeader className="py-4"><CardTitle className="text-[10px] font-black uppercase text-gray-500 tracking-[0.3em]">Opening Balance</CardTitle></CardHeader>
                    <CardContent><p className="text-3xl font-black text-white opacity-80">₹{syncData.openingBalance?.toLocaleString()}</p></CardContent>
                </Card>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="p-6 bg-red-500/5 rounded-3xl border border-red-500/10 flex items-center justify-between">
                    <div className="space-y-1">
                        ...
                        <p className="text-xl font-bold text-white">{syncData.maxDrawdownPct}%</p>
                    </div>
                ...
                <div className="p-6 bg-amber-500/5 rounded-3xl border border-amber-500/10 flex items-center justify-between">
                    <div className="space-y-1">
                        ...
                        <p className="text-xl font-bold text-white">{syncData.dailyLossPct}%</p>
                    </div>
                ...
                <div className="p-6 bg-blue-500/5 rounded-3xl border border-blue-500/10 flex items-center justify-between">
                    <div className="space-y-1">
                        ...
                        <p className="text-xl font-bold text-white">{syncData.perTradePct}%</p>
                    </div>
                    <Target className="w-5 h-5 text-blue-500 opacity-30" />
                </div>
            </div>

            {/* Hub Command Module */}
            <div className="p-6 bg-primary/5 rounded-[40px] border border-primary/20 shadow-2xl relative overflow-hidden">
                ...
                <div className="relative z-10 space-y-6">
                    ...
                    <div className="flex flex-wrap gap-3">
                        <Button 
                            ...
                            size="lg" 
                            onClick={() => {
                                const bal = prompt("Enter Start Balance for Reset:", syncData.balance);
                        ...
                        <Button 
                            ...
                            size="lg" 
                            onClick={() => {
                                const bal = prompt("Enter New Balance:", syncData.balance);
                                const hwm = prompt("Enter New HWM:", syncData.highWaterMark);
...
export default function StockmintInfoPage({ params }: { params: Promise<{ id: string, accountId: string }> }) {
  ...
  if (isFetching) return <div className="h-screen flex items-center justify-center bg-slate-950"><Loader2 className="animate-spin h-8 w-8 text-primary opacity-30"/></div>;
