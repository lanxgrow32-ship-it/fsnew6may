...
export function ArenaView({ 
    ...
    const handleMarketSwitch = (market: 'indian' | 'forex') => {
        ...
        // Force default tab based on market selection
        if (market === 'forex') setActiveTab('twoStep');
        else setActiveTab('instant');
    }

    const handleStartTrial = async () => {
        setIsStartingTrial(true);
        const res = await startFreeTrial(profile.id);
        if (res.error) toast({ title: "Trial Failed", description: res.error, variant: "destructive" });
    ...
    const PlanBox = ({ plan, category }: { plan: any, category: string }) => (
        <Card className={cn(
            ...
            category.includes('Pro') && "border-primary/20 bg-primary/[0.02]"
        )}>
            ...
            <CardContent className="flex-grow space-y-4">
                <div className="space-y-2 text-xs border-t border-white/5 pt-4">
                    <div className="flex items-center gap-2 text-muted-foreground font-medium">
                        ...
                        <span>Target: {category === 'Instant' || category.includes('Pro') ? '0%' : category.includes('PTP') ? '6%' : '10%'}</span>
                    </div>
                    <div className="flex items-center gap-2 text-muted-foreground font-medium">
                        <CheckCircle className="h-3.5 w-3.5 text-green-500" />
                        <span>{category.includes('Pro') ? '7-Day Validity' : 'Unlimited Time'}</span>
                    </div>
                </div>
    ...
    if (checkoutStep !== 'selection') {
        // ... (Checkout views same as previous turning for brevity, ensuring "Guide" is removed from parents)
        return (
        ...
        <div className="space-y-12 animate-in fade-in duration-500 pb-20">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div className="space-y-1">
                    <h2 className="text-2xl font-bold text-white tracking-tight uppercase">{marketSegment === 'forex' ? 'Forex Market' : 'Indian Market'}</h2>
                ...
                <div className="bg-black/40 border border-white/10 rounded-2xl p-1 flex items-center h-12 shadow-2xl">
                    <button onClick={() => handleMarketSwitch('indian')} className={cn("flex items-center gap-2 px-6 h-10 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all", marketSegment === 'indian' ? "bg-primary text-white shadow-lg" : "text-gray-500 hover:text-gray-300")}><LayoutGrid className="w-3.5 h-3.5" />Indian Market</button>
                    <button onClick={() => handleMarketSwitch('forex')} className={cn("flex items-center gap-2 px-6 h-10 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all", marketSegment === 'forex' ? "bg-primary text-white shadow-lg" : "text-gray-500 hover:text-gray-300")}><Globe className="w-3.5 h-3.5" />Forex Market</button>
                </div>
            </div>
                ...
                <section className="animate-in fade-in slide-in-from-top-4 duration-700">
                    <div className="max-w-6xl mx-auto border-2 border-primary/30 bg-primary/5 rounded-[40px] p-8 md:p-10 relative overflow-hidden">
                        ...
                        <div className="flex flex-col md:flex-row items-center gap-6 mb-10 relative z-10">
                            <div className="h-12 w-12 rounded-2xl bg-primary/20 text-primary flex items-center justify-center border border-primary/30 shadow-[0_0_20px_rgba(139,44,245,0.2)]"><Zap className="h-6 w-6" /></div>
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                <TabsList className="grid w-full grid-cols-2 md:grid-cols-4 max-w-2xl mx-auto h-auto p-1 bg-black/40 border border-white/10 rounded-2xl mb-10">
                    {marketSegment === 'indian' ? (
                        <>
                        ...
                        <>
                            ...
                            <div className="opacity-20 pointer-events-none flex items-center justify-center text-[10px] font-bold uppercase">More Soon</div>
                        </>
                    )}
                </TabsList>

                <TabsContent value="instant" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                    {marketSegment === 'indian' ? plans.instant.map(p => <PlanBox key={p.title} plan={p} category="Instant" />) : <div className="col-span-full py-20 text-center"><Zap className="h-10 w-10 text-primary mx-auto opacity-20 mb-4"/><h3 className="text-xl font-bold uppercase">Forex Instant Coming Soon</h3></div>}
                </TabsContent>
                <TabsContent value="twoStep" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                    {marketSegment === 'forex' ? plans.forex.map(p => <PlanBox key={p.title} plan={p} category="Forex 2-Step" />) : plans.twoStep.map(p => <PlanBox key={p.title} plan={p} category="2-Step" />)}
                </TabsContent>
                {/* ... other contents similarly updated */}
            </Tabs>
        </div>
    );
}
