        ...
        <div className="space-y-12 max-w-4xl mx-auto animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <GlassCard className="p-8 border-primary/20 bg-primary/5 h-full flex flex-col justify-center">
                    <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Current balance</p>
                    <h3 className="text-5xl font-black text-white mt-2 tracking-tighter">₹{Number(profile.wallet_balance).toLocaleString('en-IN')}</h3>
                    <div className="mt-10 p-4 bg-white rounded-2xl shadow-2xl w-fit mx-auto md:mx-0">
                        {paymentSettings?.qr_code_url ? (
                            <Image src={paymentSettings.qr_code_url} alt="Payment QR" width={220} height={180} className="rounded-lg" />
                        ) : (
                ...
                <GlassCard className="h-full">
                    <form onSubmit={handleTopUp} className="h-full flex flex-col">
                        <CardHeader>
                            <CardTitle className="text-xl font-bold flex items-center gap-2 text-white"><Wallet className="w-5 h-5 text-primary"/> Submit Deposit</CardTitle>
                            <CardDescription className="text-gray-400 font-medium">Transfer funds and enter details below.</CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-6 flex-grow">
                            <div className="space-y-4">
                                ...
                                <div className="space-y-2">
                                    <Label className="text-xs font-bold uppercase tracking-widest text-gray-500">UTR / Transaction ID</Label>
                            ...
                            <div className="p-4 bg-black/20 rounded-xl border border-white/5 flex items-center justify-between">
                                <div className="space-y-1">
                                    <p className="text-[10px] font-bold text-gray-600 uppercase tracking-widest">Transfer To</p>
                                    <p className="font-mono text-white text-sm font-bold">{paymentSettings?.upi_id || 'payout@fundedstock'}</p>
                                </div>
                        ...
                        <CardFooter>
                            <Button type="submit" disabled={isPending || !amount || !utr} className="w-full h-14 font-black uppercase tracking-widest bg-primary text-white rounded-2xl shadow-xl shadow-primary/20">
            ...
            <p className="text-center text-[10px] font-black text-gray-700 uppercase tracking-[0.3em] mt-8">
                Protocol: Manual Verification Required · Est. Time 15-30m
            </p>
        </div>
    );
}