    ...
    return (
        <div className="space-y-8 max-w-4xl mx-auto animate-in fade-in duration-500">
            <div className="space-y-1">
                ...
                <p className="text-gray-400 text-sm font-medium">Add funds for instant account activations and competition entry fees.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <GlassCard className="p-8 border-primary/20 bg-primary/5 h-full flex flex-col justify-center text-center md:text-left">
                    <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-8">
                        ...
                        <Badge variant="outline" className="bg-green-500/10 text-green-400 border-green-500/20 py-1.5 px-3 rounded-full flex items-center gap-2 font-bold">
                            <Sparkles className="w-3.5 h-3.5" /> 5% Cash Bonus
                        </Badge>
                    </div>
                    ...
                    <div className="flex flex-col items-center justify-center gap-6">
                        ...
                        <div className="w-full max-w-[240px] space-y-2">
                             <p className="text-[10px] text-gray-600 font-bold uppercase tracking-widest text-center">Payment Destination</p>
                ...
                <GlassCard className="h-full">
                    <form onSubmit={handleTopUp} className="h-full flex flex-col">
                        <CardHeader>
                            ...
                            <CardDescription className="text-gray-400 font-medium text-xs">Minimum ₹10,000 required for wallet deposits to qualify for instant status.</CardDescription>
                        </CardHeader>
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    {isBelowMin && (
                                        <p className="text-[10px] font-bold text-red-400 flex items-center gap-1 uppercase tracking-tighter">
                                            <AlertCircle className="w-3 h-3" /> Deposits below ₹10,000 are not permitted for wallet funding.
                                        </p>
                                    )}
                                </div>
                                <div className="space-y-1.5">
                                    <Label className="text-[11px] font-bold text-gray-600 uppercase">Transaction ID (UTR)</Label>
                                    <Input placeholder="Enter 12-digit reference number" value={utr} onChange={(e) => setUtr(e.target.value)} required className="bg-black/20 border-white/10 text-white h-12 text-sm font-mono" />
                                </div>
                            </div>
                        </CardContent>
                        <CardFooter className="pt-0 pb-8">
                            <Button type="submit" disabled={isPending || !amount || !utr || isBelowMin} className="w-full h-12 font-bold bg-primary text-white rounded-xl shadow-xl shadow-primary/20 text-xs uppercase tracking-widest">
            ...
            <p className="text-center text-[10px] font-bold text-gray-800 uppercase tracking-[0.2em] mt-8">
                Secured Financial Gateway · Verified Transfers Only
            </p>
        </div>
    );
}